...
JOB3=$(sbatch --parsable --export=ALL,ID=$main_ID $SLURM/03_cluster.sbatch) #--dependency=afterok:$JOB2 $SLURM/03_cluster.sbatch)
echo "  Job ID: $JOB3"
...
JOB4=$(sbatch --parsable --export=ALL,ID=$cluster_ID,IDENT=$cluster --dependency=afterok:$JOB3 $SLURM/04_subcluster.sbatch)
echo "  Job ID: $JOB4"
